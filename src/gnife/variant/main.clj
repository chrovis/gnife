(ns gnife.variant.main
  (:require [clj-sub-command.core :as cmd]
            [clojure.string :as string]
            [gnife.common :refer [error-msg exit]]
            [gnife.variant.liftover :as variant.liftover]
            [gnife.variant.to-hgvs :as variant.to-hgvs]))

(def options
  [["-h" "--help" "Print help"]])

(def commands
  [["liftover" variant.liftover/description]
   ["to-hgvs" variant.to-hgvs/description]])

(defn usage [options-summary commands-summary]
  (->> ["Usage: gnife variant [--help] <command> [<args>]"
        ""
        "Options:"
        options-summary
        ""
...
(defn exec
  ...
  (let [{:keys [command arguments exit-message ok?]} (validate-args args)]
    (if exit-message
      ...
      (case command
        :liftover (variant.liftover/exec arguments)
        :to-hgvs (variant.to-hgvs/exec arguments)))))
